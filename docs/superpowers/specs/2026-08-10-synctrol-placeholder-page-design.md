...
- **D 几何漂移**：三个大尺寸几何图形（55-60vmin 方块、60vmin 高三角形）从视口外侧单向线性横穿视口（14-18s 循环），画面中只露部分边缘。每次加载基于时间随机数决定：流动方向（左右随机，禁止全部同侧）、初始水平/垂直位置偏移、初始旋转角度（-180°~180°），并动态注入对应 `@keyframes`；无 JS 时回退到 CSS 默认动画
